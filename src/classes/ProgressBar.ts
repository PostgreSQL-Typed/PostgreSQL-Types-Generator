...
export class ProgressBar {
	...
	constructor(options: ProgressBarOptions) {
		const {
			progress: {
				...
				steps,
				line1: progressLine1,
				spinnerColor: progressSpinnerColor
			},
		...
		this.waiterFrames = [
			getConsoleHeader(
				line1 + waiterSpinner[0],
				line2,
				doubleBackslash ?? false,
				line3
			),
			getConsoleHeader(
				line1 + waiterSpinner[1],
				line2,
				doubleBackslash ?? false,
				line3
			),
			getConsoleHeader(
				line1 + waiterSpinner[2],
				line2,
				doubleBackslash ?? false,
				line3
			),
			getConsoleHeader(
				line1 + waiterSpinner[3],
				line2,
				doubleBackslash ?? false,
				line3
			)
	...
	public incrementProgress(value: number = 1) {
		this.value += value;
	...
	private getETAString() {
		...
		if (t > 3600) {
			return (
				this.autopadding(Math.floor(t / 3600), 2) +
				"h" +
				this.autopadding(round((t % 3600) / 60), 2) +
				"m"
			);
			...
			return (
				this.autopadding(Math.floor(t / 60), 2) +
				"m" +
				this.autopadding(round(t % 60), 2) +
				"s"
			);

			// > 10s ?
		} else if (t > 10) {
			return this.autopadding(round(t), 2) + "s";

			// default: don't apply round to multiple
		} else {
			return this.autopadding(t, 2) + "s";
		}
	}
	...
	private getProgressString() {
		...
		return (
			completeString.substring(0, completeSize) +
			incompleteString.substring(0, incompleteSize)
		);
	}
