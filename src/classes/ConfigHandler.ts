...
export class ConfigHandler {
	...
	public async loadConfig(): Promise<this> {
		...
		if (!parseResult.success) {
			this.LOGGER(
				`Config file is not valid ${JSON.stringify(parseResult.error.errors)}`
			);
	...
	public async initConfig(): Promise<0 | 1> {
		...
		if (config && !config.isEmpty) {
			...
			console.log(
				getConsoleHeader(
					y(
						"A configuration file is already present, skipping initialization..."
					),
					`File location: ${config.filepath}`
				)
			);
			return 0;
		}
