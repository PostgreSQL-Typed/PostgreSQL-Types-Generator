...
export class Printer {
				...
				.flatMap(d =>
					d.classes.filter(c =>
						d.tables
							...
							.includes(`${c.schema_name}.${c.class_name}`)
					)
				)
				.map(c => [c.class_id, c])
		);
	...
	private _getTypeOverride(type: DataType): string | null {
		return (
			this.config.types.typeOverrides[
				`${type.schema_name}.${type.type_name}`
			] ??
			this.config.types.typeOverrides[`${type.type_name}`] ??
			null
		);
	}
	...
	private _getZodOverride(type: DataType): string | null {
		return (
			this.config.types.zod.zodOverrides[
				`${type.schema_name}.${type.type_name}`
			] ??
			this.config.types.zod.zodOverrides[`${type.type_name}`] ??
			null
		);
	}
	...
	public async print() {
		const allClasses = [...this.classes.values()].filter(
				cls => cls.kind === ClassKind.OrdinaryTable
			),
			dbClassLists: ClassDetails[][] = [];

		for (const cls of allClasses) {
			const classList = dbClassLists.find(
				c => c[0].database_name === cls.database_name
			);
		...
		if (this.config.types.zod.enabled)
			for (const classList of dbClassLists) printDatabaseZod(classList, this);

		return await this.writeFiles();
	}

	private async writeFiles() {
		const directory = this.config.types.directory,
			...
			files.map(async f => {
				...
				if (filename.endsWith(`.ts`)) {
					const content = f.content.trim(),
						checksum = `Checksum: ${createHash("sha512")
							.update(content)
							.digest("base64")}`;
