	...
	public pushDeclaration(
		...
		if (!this._declarationNames.has(identifierName)) {
			...
			const declarationLines = declaration(identifierName, {
				...
				addStringImport: (importString: string) =>
					this._addStringImport(importString)
			});
		...
		return {
			mode,
			file: this.file,
			hasPrimaryKey: this._declarations.some(line =>
				line().some(l => l.startsWith("export const PrimaryKey"))
			),
