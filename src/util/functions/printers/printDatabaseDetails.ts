...
import { printSchemaDetails } from "../../functions/printers/printSchemaDetails";
...
export function printDatabaseDetails(types: ClassDetails[], printer: Printer) {
	if (types.some(type => type.kind !== ClassKind.OrdinaryTable))
		throw new Error(
			"printDatabaseDetails only supports ordinary tables at the moment."
		);

	const schemaClassList: ClassDetails[][] = [];

	for (const cls of types) {
		const list = schemaClassList.find(
			c => c[0].schema_name === cls.schema_name
		);
