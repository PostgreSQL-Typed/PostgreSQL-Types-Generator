...
export function printClassZod(type: ClassDetails, printer: Printer) {
	if (type.kind !== ClassKind.OrdinaryTable)
		throw new Error(
			"printClassZod only supports ordinary tables at the moment."
		);
	...
	const zInsertParameters = printer.context.pushValueDeclaration(
		...
		(identifierName, file) => [
			...
			type.attributes
				...
				.map(attribute => [
					`  ${attribute.attribute_name}: ${getAttributeZod(
						type,
						attribute,
						printer,
						file
					)}${optionalOnInsert(attribute)}`
				])
				.reduce((a, b) => [...a, ...b], [])
				.join(",\n"),
			`});`
...
function getAttributeZod(
	...
	if (!attribute.not_null)
		return `z.nullable(${getAttributeZod(
			type,
	...
	const columnTypeOverride =
		printer.config.types.zod.columnZodOverrides[
			...
			if (
				...
				if (referencedClass) {
					...
					if (referencedAttribute) {
						const { zInsertParameters } = printClassZod(
							referencedClass,
							printer
						);

						return `${file.getImport(zInsertParameters)}['${
