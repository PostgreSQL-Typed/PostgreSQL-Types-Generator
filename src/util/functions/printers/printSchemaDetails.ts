...
export function printSchemaDetails(types: ClassDetails[], printer: Printer) {
	if (types.some(type => type.kind !== ClassKind.OrdinaryTable))
		throw new Error(
			"printSchemaDetails only supports ordinary tables at the moment."
		);
	...
	const SchemaTypeRecord = printer.context.pushTypeDeclaration(
		...
		(identifier, { getImport }) => [
			...
			...types
				...
				.map(cls => {
					const { DatabaseRecord, InsertParameters, PrimaryKey } =
						printClassDetails(cls, printer);
					return `    ${cls.class_name}: {\n      name: "${
						cls.class_name
					}";\n      primary_key: typeof ${getImport(
						PrimaryKey
					)};\n      columns: ${getImport(
						DatabaseRecord
					)};\n      insert_parameters: ${getImport(
						InsertParameters
					)};\n    };`;
				}),
			`  };`,
