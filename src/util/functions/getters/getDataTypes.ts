...
export async function getDataTypes(client: Client, schema_names: string[]) {
	const conditions = schema_names.map(
		schema_name => `ns.nspname = '${schema_name}'`
	);

	const { rows: typeRecords } = await client.query<DataTypeRecord>(`
    SELECT
