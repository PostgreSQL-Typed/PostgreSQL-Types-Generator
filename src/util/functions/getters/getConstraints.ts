...
export async function getConstraints(client: Client, schema_names: string[]) {
	const conditions = [
		`ns.nspname IN (${schema_names.map(s => `'${s}'`).join(`, `)})`
	];

	const { rows: constraints } = await client.query<Constraint>(`
    SELECT
