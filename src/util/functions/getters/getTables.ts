	...
	return rows
		.filter(t => {
			...
			if (typeof schemas === "string")
				return schemas === `${dbName}.${t.schema_name}`;
			if (typeof schemas === "number") return schemas === t.schema_id;

			return (
				[...schemas]
					.filter((s): s is string => typeof s === "string")
					.includes(`${dbName}.${t.schema_name}`) ||
				[...schemas]
					.filter((s): s is number => typeof s === "number")
					.includes(t.schema_id)
			);
		})
		.filter(t => {
			const { tables } = config;
			...
			if (typeof tables === "string")
				return tables === `${dbName}.${t.schema_name}.${t.table_name}`;
			if (typeof tables === "number") return tables === t.table_id;

			return (
				[...tables]
					.filter((s): s is string => typeof s === "string")
					.includes(`${dbName}.${t.schema_name}.${t.table_name}`) ||
				[...tables]
					.filter((s): s is number => typeof s === "number")
					.includes(t.table_id)
			);
